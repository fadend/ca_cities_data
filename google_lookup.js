...
async function lookUpLatLng() {
  ...
  const failedArea = document.getElementById("failed");
  ...
  let results = [];
  let failed = [];
  for (let city of CA_CITIES) {
    if (results.length > 0 || failed.results > 0) {
      await wait(500 + parseInt(500 * Math.random()));
    }
    let places = [];
    try {
      let response = await google.maps.places.Place.searchByText({
        textQuery: `${city}, CA, USA`,
    ...
    if (places.length == 0) {
      ...
      failed.push(city);
      failedArea.value = failed.join("\n");
    } else if (places.length > 1) {
      console.log("Too many results for", city, places);
      failed.push(city);
      failedArea.value = failed.join("\n");
    } else {
      let place = places[0];
      ...
      resultArea.value = JSON.stringify(results);
    }
  }
}
